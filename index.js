...
//CALCULO FECHA Y MISMA FRASE TODO EL DÍA FRASE 1 DÍA 1 DEL AÑO
    ...
    // Se activan al decir tablero : hola y buenos días (Ej.: tablero hola y tablero buenos días una señal)
    const esApertura = texto.includes("hola") 
              || texto.includes("buenos dias");
          
      const frase = FRASES_DIA[diaDelAño % FRASES_DIA.length];
      return res.json(respuestaAlexa(frase)); // 🛑 AQUÍ CORTA Y AHORRA
...
// Se activan al decir tablero : señal, orden, ayuda o peligro (Ej.: Necesito una señal)
    ...
    return res.json(respuestaAlexa(mensaje)); // 🛑 AQUÍ CORTA Y AHORRA
